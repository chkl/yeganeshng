...
import Data.Map (Map, insert, findWithDefault, fromList, toList, union)
...
data Options = Options { dmenuOpts :: [String], profile :: String, prune :: Bool, executables :: Bool }
data Flag    = Profile String | Prune | Executables | Version | Help deriving (Eq, Ord, Show, Read)

compactFlags :: [Flag] -> (Flag, Bool, Bool)
compactFlags fs = (flag, not $ null prunes, not $ null execs) where
...
readPossiblyNonExistent file = catch (readUTF8File file) (const . return $ "") >>= parseCurrentFormat where
...
merge a b = postscale (on geometricMean (prescale . clip) a b) where
    prescale    = (0.5 +) . (/ 2)
